...
class Service:
    def _init_(self, date, operation, price):
        self.date = date
...
class Car:
    def _init_(self, name, model, bought, price, boughtplace):
        self.nickname = name
        self.model = model
        self.bought = bought
        self.price = price
    ...
    def _str_(self):
        ...
        return (f"Name:  {self.nickname}\n"
                f"Brand: {self.model.brand} {self.model.model} Bought: {self.bought.strftime('%d.%m.%Y')}\n"
                f"Service history: \n{services_str}\n"
                f"Bought place:  {self.boughtplace}")
